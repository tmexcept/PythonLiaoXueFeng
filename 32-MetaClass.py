...
# 虽然云里雾里，但是有几个地方还是要注意下的。
# 1、定义Model里的__getattr__和__setattr__是为了将dict用key,value处理成对象的属性关系。
# 2、定义ModelMetaclass里的attrs.pop(k)是为了获取User实例初始化的赋值，
#    否则我们可以看到print('ARGS: %s' % str(args))输出的不是ARGS: ['my-pwd', 'test@orm.org', 'Michael', 12345],
#    而是IntegerField和StringField对象，原因就是我们创建User的时候设置的属性，这些属性存在所以是不会调用__getattr__方法的，所以要attrs.pop(k)
